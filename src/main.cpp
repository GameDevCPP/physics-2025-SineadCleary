...
unsigned int gameHeight = 800.f;
unsigned int gameWidth = 800.f;
b2World* world;
...
void init() {
    ...
    for (int i = 0; i < 7; i += 2) {
        ...
        s->setPosition(Vector2f(walls[i]));
        s->setSize(walls[i+1]);
...
int main() {
    auto window = sf::RenderWindow{ { gameWidth, gameHeight }, "CMake SFML Project" };
